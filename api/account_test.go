...
func TestGetAccountAPI(t *testing.T) {
	...
	for index := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			defer ctr.Finish() // it will check to see if all methods that were expected to be called were called.
			...
			recorder := httptest.NewRecorder() // New ResponseRecorder: get the record of test HTTP response instead of calling the real HTTP server
	...
	testCases := []struct {
	...
	}{
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				...
				store.EXPECT().UpdateAccount(gomock.Any(), gomock.Eq(arg)).Times(1).Return(account, nil)
			},
			checkResponse: func(recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusOK, recorder.Code)
				requireBodyMatchAccount(t, recorder.Body, account)
